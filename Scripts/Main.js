...
function UpdateScene(CurrentTime)
{
    ...
    if (Time == 0)
    {
        ...
        BackroundMusic.play(); 
        BackroundMusic.volume = 0.5; // Set music volume
...
function FadeAudio()
{
    ...
    else
    {
        ...
        BackroundMusic.volume = Math.min(0.5, BackroundMusic.volume + (0.5 / 30));
...
// Add this near your other DOM element selections
const likeButton = document.getElementById('likeButton');
const heartIcon = document.querySelector('.heart-icon');
const likeCount = document.getElementById('likeCount');
const likeInput = document.getElementById('likeName');
const submitLike = document.getElementById('submitLike');
const likeInputContainer = document.querySelector('.like-input-container');
...
likeButton.addEventListener('click', () => {
    if (!heartIcon.classList.contains('active')) {
        likeInputContainer.classList.add('show');
    }
});

submitLike.addEventListener('click', async () => {
    const name = likeInput.value.trim();
    if (name) {
        try {
            ...
            likeInputContainer.classList.remove('show');
            likeInput.value = '';

            // Show success message
            const successMessage = document.createElement('div');
            successMessage.textContent = 'Thanks for your like!';
            successMessage.style.color = '#4CAF50';
            successMessage.style.fontSize = '1.4vh';
            likeInputContainer.parentNode.insertBefore(successMessage, likeInputContainer.nextSibling);
            setTimeout(() => successMessage.remove(), 3000);
        } catch (error) {
            console.error('Error:', error);
            const errorMessage = document.createElement('div');
            errorMessage.textContent = 'Failed to send like. Please try again.';
            errorMessage.style.color = '#ff4444';
            errorMessage.style.fontSize = '1.4vh';
            likeInputContainer.parentNode.insertBefore(errorMessage, likeInputContainer.nextSibling);
